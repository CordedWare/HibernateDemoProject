...
class UserDaoTest {
    ...
    private final SessionFactory sessionFactory = HibernateTestUtil.buildSessionFactory(); // иницилизирую сессию 1 раз, чтобы каждый раз не поднимать для нового теста
    private final UserDao userDao = UserDao.getInstance(); // от него идут все наши запросы

    @BeforeAll // инициализирую сущности
    public void initDb() {
        TestDataImporter.importData(sessionFactory);
    }

    @AfterAll // после выполнения всех наших тестов я закрываю сессию
    public void finish() {
